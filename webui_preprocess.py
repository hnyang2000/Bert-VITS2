        ...
        with gr.Row():
            with gr.Column():
                ...
                _ = gr.Markdown(
                    ...
                    "- 部署：修改根目录下的 `config.yml` 中 `webui` 下 `model` 一项为 `models/{权重文件名}.pth` （如 models/leidian/models/G_4600.pth），修改`config_path` 一项为 `models/{配置文件名}.json` （如 models/leidian/config.json），然后执行 `python webui.py`"
                )
